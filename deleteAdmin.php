<?php 
	...
	if (isset($_GET['did'])) {//did is get from the id
		$stmt =$pdo->prepare("DELETE  FROM tb_admin where id=:did");//delete from table admin whenever the id equals to did

		if($stmt->execute($_GET)){//execution GET method did
			header('Location:addAdmin.php');//location goes to addAdmin.php after execution
		}
